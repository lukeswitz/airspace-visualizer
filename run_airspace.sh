...
adsb_cmd() {
  if [[ -n "${READSB_BIN}" ]]; then
    echo "${READSB_BIN} --device-type rtlsdr --device ADSB --write-json /tmp --write-json-every 1"
  elif [[ -n "${DUMP1090_FA_BIN}" ]]; then
    echo "${DUMP1090_FA_BIN} --net --write-json /tmp --write-json-every 1"
  elif [[ -n "${DUMP1090_MUT_BIN}" ]]; then
    echo "${DUMP1090_MUT_BIN} --net --write-json /tmp --write-json-every 1"
  elif [[ -n "${DUMP1090_BIN}" ]]; then
    echo "${DUMP1090_BIN} --net --write-json /tmp --write-json-every 1"
  else
    echo ""
  fi
}
...
vdl2_cmd() {
  if [[ -n "${DUMPVDL2_BIN}" ]]; then
    echo "${DUMPVDL2_BIN} --rtlsdr VDL2 136650000 136725000 136775000 136800000 136825000 136875000 136900000 136975000 --output decoded:json:file:path=\"${VDL2_NDJSON}\""
  else
    echo ""
  fi
}
